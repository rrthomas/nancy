...
\usepackage{babel,a4,newlfont,fancyvrb,copyrght,graphicx,url}
...
\\User’s guide}
\date{27th March 2016} % FIXME: have this date automatically filled in by git
...
Nancy is written in Perl, and requires version 5.10 or later, and the \verb|RRT::Macro| module version 3 or later (available from \url{https://github.com/rrthomas/perl}.
...
\item Scan the text for commands. Weave any arguments to the command, run each command, and replace the command by the result.
...
\item[\UseVerb{include}]Look up the given file. If it is executable, run it as a command with the given arguments and collect the output. Otherwise, read the contents of the given file. Weave and return the result.
\SaveVerb{paste}|$paste{FILE, ARGUMENT, …}|
\item[\UseVerb{paste}]Like \verb|$include|, but does not weave its result before returning it.
\SaveVerb{path}|$path|
...
\item Try looking for file on the user’s \verb|PATH| (the list of directories specified by the \verb|PATH| environment variable).
