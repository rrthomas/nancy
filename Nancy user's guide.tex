...
fragment evaluated as a Perl expression, which is expected to produce a subroutine, which is then called with the given arguments, and the directory in which the current fragment is found as an extra final argument, represented as a reference to an array of path components, left-to-right.
\end{description}
