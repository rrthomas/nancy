...
\item[\UseVerb{root}]Replace the command with the relative URL from the page currently being constructed to the root of the site. This means that every link in a site can written relative to the current page, either explicitly (which makes sense for pages related to the current page), or implicitly as \UseVerb{rellink}. Hence the site's base URL can be changed without needing to change any intra-site links.
