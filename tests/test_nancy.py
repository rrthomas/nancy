...
async def test_update_with_existing_input(chtestdir) -> None:
...
async def test_update_overwriting_some_input(chtestdir) -> None:
    # Create temporary directory to copy initial files into
    with TemporaryDirectory() as tmp_dir:
...
async def test_env_vars(chtestdir) -> None:
    await passing_test("env-vars-src", "env-vars-expected")


async def test_nested_macro_invocations(chtestdir) -> None:
    await passing_test("nested-macro-src", "nested-macro-expected")


async def test_expansion_while_including_from_subdirs(chtestdir) -> None:
    await passing_test("combine-subincludes-src", "combine-subincludes-expected")


async def test_expansion_of_symlinked_directories(chtestdir) -> None:
    await passing_test("symlink-directory-src", "symlink-directory-expected")


async def test_expand_of_run_output(chtestdir) -> None:
    await passing_test("expand-run-src", "expand-run-expected")


async def test_expanding_directory_name(chtestdir) -> None:
    await passing_test("expand-directory-name-src", "expand-directory-name-expected")


async def test_nonexistent_executable_test(chtestdir) -> None:
    await failing_test(os.getcwd(), "cannot find program 'foo'", "foo.nancy.txt")


async def test_passing_executable_test(chtestdir) -> None:
    await passing_test(os.getcwd(), "true-expected.txt", "true.nancy.txt")


async def test_executable_test(chtestdir) -> None:
    await passing_test(
        "page-template-with-date-src", "page-template-with-date-expected"
    )


async def test_executable_in_cwd_test(chtestdir) -> None:
    await passing_test(
        ".", "executable-in-cwd-expected.txt", "executable-in-cwd.nancy.txt"
    )


async def test_hidden_files_are_ignored(chtestdir) -> None:
...
async def test_hidden_files_can_be_processed(chtestdir) -> None:
    await passing_test(
        "hidden-files",
        "hidden-files",
        None,
...
async def test_macros_not_expanded_in_command_line_arguments(chtestdir) -> None:
    await passing_test("$path-src", "$path-expected")


async def test_path_in_filename(chtestdir) -> None:
    await passing_test("path-in-filename-src", "path-in-filename-expected")


async def test_outputpath_in_filename(chtestdir) -> None:
    await failing_test(
...
async def test_copy_suffix(chtestdir) -> None:
...
async def test_not_delete_ungenerated(chtestdir) -> None:
    # Create temporary directory to copy initial files into
    with TemporaryDirectory() as tmp_dir:
        shutil.copytree("webpage-src", tmp_dir, dirs_exist_ok=True)
...
async def test_paste_does_not_expand_macros(chtestdir) -> None:
    await passing_test("paste-src", "paste-expected")


async def test_path_with_arguments_gives_an_error(chtestdir) -> None:
    await failing_test(
        os.getcwd(),
...
async def test_path_with_input_gives_an_error(chtestdir) -> None:
    await failing_test(
        os.getcwd(),
...
async def test_outputpath_with_arguments_gives_an_error(chtestdir) -> None:
    await failing_test(
        os.getcwd(),
...
async def test_outputpath_with_input_gives_an_error(chtestdir) -> None:
    await failing_test(
        os.getcwd(),
...
async def test_include_with_no_arguments_gives_an_error(chtestdir) -> None:
    await failing_test(
        os.getcwd(),
...
async def test_include_with_input_gives_an_error(chtestdir) -> None:
    await failing_test(
        os.getcwd(),
...
async def test_paste_with_no_arguments_gives_an_error(chtestdir) -> None:
    await failing_test(
        os.getcwd(),
...
async def test_paste_with_input_gives_an_error(chtestdir) -> None:
    await failing_test(
        os.getcwd(),
...
async def test_paste_with_too_many_arguments_gives_an_error(chtestdir) -> None:
    await failing_test(
        os.getcwd(),
        "$paste needs exactly one argument",
...
async def test_expand_with_arguments_gives_an_error(chtestdir) -> None:
    await failing_test(
        os.getcwd(),
...
async def test_expand_without_input_gives_an_error(chtestdir) -> None:
    await failing_test(
        os.getcwd(),
...
async def test_run_with_no_arguments_gives_an_error(chtestdir) -> None:
    await failing_test(
        os.getcwd(),
...
async def test_update_run_with_no_arguments_gives_an_error(chtestdir) -> None:
    await failing_test(
        os.getcwd(),
        "$run needs at least one argument",
        "run-no-arg.nancy.txt",
...
async def test_escaping_a_macro_without_arguments(chtestdir) -> None:
    await passing_test("escaped-path-src", "escaped-path-expected")


async def test_escaping_a_macro_with_arguments(chtestdir) -> None:
    await passing_test("escaped-include-src", "escaped-include-expected")


async def test_cookbook_web_site_example(chtestdir) -> None:
    await passing_test(
...
async def test_expanding_a_file_with_relative_includes(chtestdir) -> None:
    await passing_test(
        os.getcwd(),
...
async def test_nested_argument_in_comma_list_works(chtestdir) -> None:
    await passing_test(
...
async def test_expanding_macros_in_file_names(chtestdir) -> None:
    await passing_test(
...
async def test_expanding_macro_in_single_file_argument_with_directory_output(
    chtestdir,
) -> None:
    with TemporaryDirectory() as tmp_dir:
        await passing_test(
...
async def test_run_with_input(chtestdir) -> None:
    await passing_test(
        os.getcwd(),
        "lines-expected.txt",
        "filter.nancy.txt",
...
async def test_run_with_input_containing_commas(chtestdir) -> None:
    await passing_test(
        os.getcwd(),
...
async def test_a_nonexistent_input_path_causes_an_error(chtestdir) -> None:
    await failing_test("a", "input 'a' does not exist")


async def test_an_input_that_is_not_a_directory_causes_an_error(chtestdir) -> None:
    await failing_test("random-text.txt", "input 'random-text.txt' is not a directory")


async def test_including_a_nonexistent_file_causes_an_error(chtestdir) -> None:
    await failing_test(os.getcwd(), "cannot find 'foo'", "missing-include.nancy.txt")


async def test_calling_an_undefined_macro_causes_an_error(chtestdir) -> None:
    await failing_test(os.getcwd(), "no such macro '$foo'", "undefined-macro.nancy.txt")


async def test_calling_an_undefined_single_letter_macro_causes_an_error(
    chtestdir,
) -> None:
    await failing_test(
...
async def test_a_macro_call_with_a_missing_close_brace_causes_an_error(
    chtestdir,
) -> None:
...
async def test_trying_to_output_multiple_files_to_stdout_causes_an_error(
    chtestdir,
) -> None:
    await failing_test(
